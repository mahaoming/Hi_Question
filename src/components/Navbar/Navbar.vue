<template>
  <view class="hx-navbar">
    <view
      :class="{
        'hx-navbar--fixed': fixed,
        'hx-navbar--shadow': shadow,
        'hx-navbar--border': border
      }"
    ...
    >
      ...
      <view
        :style="{ height: statusBarHeight, background: statusBarBackground }"
        class="hx-status-bar"
        v-if="statusBar"
      ></view>
      <view
        :style="{ color: colorInfo, height: height, 'line-height': height }"
        class="hd hx-navbar__header hx-navbar__content_view"
      >
        <view
        ...
        >
          <block v-if="leftText.length || leftIcon.length || back">
            ...
            >
              <hi-icon
                :name="back ? 'iconfontjiantouzuo' : leftIcon"
                :color="colorInfo"
                size="md"
              />
            </view>
          ...
          <block v-else>
            <slot
              name="leftAfter"
              v-if="leftSlidiSwitch && slotSlidiSwitch == 1"
            />
        ...
        <view class="hx-navbar__header-container hx-navbar__content_view">
          ...
          <block v-else>
            <slot
              name="centerAfter"
              v-if="centerSlidiSwitch && slotSlidiSwitch == 1"
            />
            <slot v-else />
          </block>
        </view>
        ...
        <view
          :class="title.length ? 'hx-navbar__header-btns-right' : ''"
          class="hx-navbar__header-btns hx-navbar__content_view"
          @tap="onClickRight"
          v-if="rightSlot"
        >
          ...
          <block v-if="rightIcon.length || rightText.length">
            <view class="hx-navbar__content_view" v-if="rightIcon.length">
              <hi-icon :name="rightIcon" :color="colorInfo" size="lg" />
            </view>
            <view
              v-if="rightText.length"
              class="hx-navbar-btn-text hx-navbar__content_view"
              >{{ rightText }}</view
            >
          </block>
          <block v-else>
            <slot
              name="rightAfter"
              v-if="rightSlidiSwitch && slotSlidiSwitch == 1"
            />
...
<script>
import hiIcon from "_c/Basic/Icon";
//获取系统状态栏高度
...
export default {
  ...
  created() {
    ...
    //文字颜色
    that.colorContainer =
      typeof that.color == "object" ? that.color : [that.color, that.color];
    ...
    if (
      that.fixed &&
      (that.leftSlidiSwitch || that.centerSlidiSwitch || that.rightSlidiSwitch)
    ) {
  ...
  watch: {
    pageScroll(val, oldVal) {
      ...
      if (
        that.fixed &&
        (that.leftSlidiSwitch ||
          that.centerSlidiSwitch ||
          that.rightSlidiSwitch)
      ) {
        that.doScroll(val);
      }
    },
    ...
    transparentValue(val, oldVal) {
      ...
      if (tt.setNavigationBarColor) {
        ...
        console.log(
          "hx-navbar 提示：当前客户端版本过低，无法使用状态栏颜色修改功能，请升级（基础库1.40+）。"
        );
      }
      // #endif
    },
  ...
  methods: {
    ...
    setBgColor() {
      ...
      if (typeof that.backgroundColor[0] == "object") {
        ...
        if (l >= 2) {
          let rgbStr =
            "linear-gradient(" + that.backgroundColorLinearDeg + "deg,";
          let c = null;
        ...
        let rgbStr =
          that.backgroundColor[0] +
          "," +
          that.backgroundColor[1] +
          "," +
          that.backgroundColor[2];
        that.backgroundColorRgb = "rgb(" + rgbStr + ")";
        that.backgroundColorRgba =
          "rgba(" + rgbStr + "," + that.transparentValue + ")";
      }
    },
    setStatusBarFontColor() {
      var that = this;
      if (typeof that.statusBarFontColor == "string") {
        that.statusBarFontColorInfo = [
          that.statusBarFontColor,
          that.statusBarFontColor
        ];
      } else if (typeof that.statusBarFontColor == "object") {
        if (that.statusBarFontColor.length == 1) {
          that.statusBarFontColorInfo = [
            that.statusBarFontColor[0],
            that.statusBarFontColor[0]
          ];
        } else if (that.statusBarFontColor.length >= 2) {
          that.statusBarFontColorInfo = [
            that.statusBarFontColor[0],
            that.statusBarFontColor[1]
          ];
        }
      }
      // #ifndef MP-TOUTIAO || H5
...
.hx-navbar {
  ...
  &--shadow {
    box-shadow: 0 2upx 12upx #cccccc;
  }
